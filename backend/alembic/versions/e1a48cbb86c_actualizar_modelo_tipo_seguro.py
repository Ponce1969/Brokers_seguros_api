...
def upgrade() -> None:
    ...
    op.add_column(
        "tipos_de_seguros", sa.Column("nombre", sa.String(100), nullable=True)
    )
    op.add_column(
        "tipos_de_seguros", sa.Column("descripcion", sa.Text(), nullable=True)
    )
    op.add_column(
        "tipos_de_seguros",
        sa.Column("es_default", sa.Boolean(), server_default="false"),
    )
    op.add_column(
        "tipos_de_seguros",
        sa.Column("esta_activo", sa.Boolean(), server_default="true"),
    )
    op.add_column(
        "tipos_de_seguros",
        sa.Column(
    ...
    op.alter_column(
        "tipos_de_seguros", "codigo", existing_type=sa.String(10), nullable=False
    )
    op.alter_column(
        "tipos_de_seguros", "nombre", existing_type=sa.String(100), nullable=False
    )

    # Crear índice único para codigo
    op.create_unique_constraint(
        "uq_tipos_de_seguros_codigo", "tipos_de_seguros", ["codigo"]
    )
