...
def upgrade() -> None:
    ...
    op.alter_column('monedas', 'descripcion',
    ...
    op.add_column('monedas', sa.Column('es_default', sa.Boolean(), server_default='false'))
    op.add_column('monedas', sa.Column('esta_activa', sa.Boolean(), server_default='true'))
    op.add_column('monedas', sa.Column('fecha_creacion', sa.DateTime(timezone=True), server_default=sa.text('now()')))
    op.add_column('monedas', sa.Column('fecha_actualizacion', sa.DateTime(timezone=True), nullable=True))
    
    # Hacer simbolo NOT NULL después de agregarlo
    op.execute("UPDATE monedas SET simbolo = SUBSTRING(codigo FROM 1 FOR 1) WHERE simbolo IS NULL")
    op.alter_column('monedas', 'simbolo',
...
def downgrade() -> None:
    ...
    op.alter_column('monedas', 'nombre',
