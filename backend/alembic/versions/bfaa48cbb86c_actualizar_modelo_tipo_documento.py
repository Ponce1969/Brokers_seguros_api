...
def upgrade() -> None:
    ...
    op.add_column(
        "tipos_documento", sa.Column("descripcion", sa.String(200), nullable=True)
    )
    op.add_column(
        "tipos_documento", sa.Column("es_default", sa.Boolean(), server_default="false")
    )
    op.add_column(
        "tipos_documento", sa.Column("esta_activo", sa.Boolean(), server_default="true")
    )

    # Crear un índice único para codigo
    op.create_unique_constraint(
        "uq_tipos_documento_codigo", "tipos_documento", ["codigo"]
    )

    # Actualizar los registros existentes con valores por defecto para codigo
    op.execute(
        "UPDATE tipos_documento SET codigo = SUBSTRING(nombre FROM 1 FOR 10) WHERE codigo IS NULL"
    )

    # Hacer codigo NOT NULL después de la actualización
    op.alter_column(
        "tipos_documento", "codigo", existing_type=sa.String(10), nullable=False
    )
