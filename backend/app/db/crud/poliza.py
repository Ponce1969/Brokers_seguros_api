...
class CRUDPoliza:
    ...
    def _apply_filters(self, query, **filters):
        ...
        if filters.get("corredor_id"):
            query = query.filter(MovimientoVigencia.corredor_id == filters["corredor_id"])
        ...
        if filters.get("fecha_inicio") and filters.get("fecha_fin"):
            query = query.filter(
                MovimientoVigencia.fecha_inicio.between(filters["fecha_inicio"], filters["fecha_fin"])
            )
        if filters.get("numero_poliza"):
            query = query.filter(MovimientoVigencia.numero_poliza.ilike(f"%{filters['numero_poliza']}%"))
        return query
    ...
    async def get_by_numero(self, db: AsyncSession, numero_poliza: str) -> Optional[MovimientoVigencia]:
        """Obtener una póliza por su número."""
        result = await db.execute(
            select(MovimientoVigencia)
            .filter(MovimientoVigencia.numero_poliza == numero_poliza)
        )
        return result.scalar_one_or_none()

    async def get_multi(self, db: AsyncSession, **filters) -> List[MovimientoVigencia]:
        """Obtener múltiples pólizas con filtros opcionales."""
        query = select(MovimientoVigencia).join(Cliente, MovimientoVigencia.cliente_id == Cliente.id)
        query = self._apply_filters(query, **filters)
        query = query.options(*self._joined_load_options()).offset(filters.get("skip", 0)).limit(filters.get("limit", 100))
        result = await db.execute(query)
        return result.scalars().all()

    async def create(self, db: AsyncSession, *, obj_in: PolizaCreate) -> MovimientoVigencia:
    ...
    async def update(self, db: AsyncSession, *, db_obj: MovimientoVigencia, obj_in: PolizaUpdate) -> MovimientoVigencia:
    ...
    async def delete(self, db: AsyncSession, *, id: int) -> Optional[MovimientoVigencia]:
    ...
    async def get_estadisticas(self, db: AsyncSession, **filters) -> Tuple[List[Dict], float, float, int]:
        ...
        totals_query = select(
            ...
            func.coalesce(func.sum(MovimientoVigencia.suma_asegurada), 0).label("suma_asegurada_total"),
        ...
        return stats_by_duration, totals.suma_asegurada_total, totals.prima_total, totals.total_polizas
