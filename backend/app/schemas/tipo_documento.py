...
class TipoDocumentoBase(BaseModel):
    ...
    nombre: str = Field(
        ..., min_length=2, max_length=50, description="Nombre del tipo de documento"
    )
    descripcion: Optional[str] = Field(
        None, description="Descripción adicional del tipo de documento"
    )
    es_default: bool = Field(
        default=False, description="Indica si es el tipo de documento por defecto"
    )
    esta_activo: bool = Field(
        default=True, description="Indica si el tipo de documento está activo"
    )
...
class TipoDocumento(TipoDocumentoBase):
    ...
    clientes_count: Optional[int] = Field(
        0, description="Cantidad de clientes que usan este tipo de documento"
    )
