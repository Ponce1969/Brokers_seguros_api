...
class AseguradoraBase(BaseModel):
    ...
    nombre: str = Field(
        ..., min_length=2, max_length=100, description="Nombre de la aseguradora"
    )
    ...
    telefono: Optional[str] = Field(
        None, min_length=8, max_length=20, description="Teléfono de contacto"
    )
    direccion: Optional[str] = Field(
        None, max_length=200, description="Dirección física"
    )
    email: Optional[EmailStr] = Field(
        None, description="Correo electrónico de contacto"
    )
    pagina_web: Optional[HttpUrl] = Field(None, description="Sitio web oficial")
    esta_activa: bool = Field(
        default=True, description="Indica si la aseguradora está activa"
    )
    observaciones: Optional[str] = Field(
        None, description="Notas adicionales sobre la aseguradora"
    )
...
class Aseguradora(AseguradoraBase):
    ...
    tipos_seguro_count: Optional[Annotated[int, Field(ge=0)]] = Field(
        0, description="Cantidad de tipos de seguro ofrecidos"
    )
...
class AseguradoraWithRelations(Aseguradora):
    ...
    tipos_seguro: List["TipoSeguro"] = Field(
        default_factory=list, description="Lista de tipos de seguro"
    )
