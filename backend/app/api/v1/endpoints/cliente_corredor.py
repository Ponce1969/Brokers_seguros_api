...
async def create_cliente_corredor(
    *, db: AsyncSession = Depends(get_db), cliente_corredor_in: ClienteCorredorCreate
) -> Any:
    """
...
async def get_cliente_corredor(
    relacion_id: int, db: AsyncSession = Depends(get_db)
) -> Any:
    """
    ...
    if not relacion:
        raise HTTPException(
            status_code=404, detail="Relación cliente-corredor no encontrada"
        )
    return relacion
...
async def update_cliente_corredor(
    *,
    ...
    relacion = await cliente_corredor_crud.update(
        db, db_obj=relacion, obj_in=cliente_corredor_in
    )
    return relacion
...
async def delete_cliente_corredor(
    *, db: AsyncSession = Depends(get_db), relacion_id: int
) -> Any:
    """
