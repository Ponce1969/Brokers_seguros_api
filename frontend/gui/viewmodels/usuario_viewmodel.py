...
class UsuarioViewModel(ViewModelBase[Usuario]):
    ...
    async def _validar_datos_usuario(self, datos: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "corredor_numero" in datos:
                numero_corredor = datos["corredor_numero"]
                if not isinstance(numero_corredor, int):
                    errores.append("El número de corredor debe ser un número entero")
                elif numero_corredor < 1000 or numero_corredor > 9999:
                    errores.append("El número de corredor debe estar entre 1000 y 9999")
                else:
                    # Verificar si el corredor existe
                    corredor_existe = await self._repositorio.verificar_corredor_existe(numero_corredor)
                    if not corredor_existe:
                        errores.append(
                            f"No existe un corredor con el número {numero_corredor}. "
                            "Debe crear primero el corredor antes de crear el usuario."
                        )
    ...
    def _preparar_datos_corredor(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "observaciones": datos_limpios.get(
                "observaciones", "Corredor creado desde la interfaz de usuario"
            ),
    ...
    async def _limpiar_corredor(self, corredor: dict) -> None:
        ...
        if corredor and "numero" in corredor:
            try:
                await self._repositorio.servicio_api.delete(
                    f"corredores/{corredor['numero']}"
                )
                logger.info(
                    f"Corredor {corredor['numero']} eliminado después de fallo en creación de usuario"
                )
    ...
    async def crear_usuario(self, **datos) -> None:
        ...
        try:
            ...
            logger.info(
                f"Iniciando creación de corredor/usuario con email: {datos['email']}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Número de corredor asignado: {corredor.get('numero', 'No asignado')}"
                )
            except Exception as e:
                error_msg = str(e)
            ...
            usuario_creado = await self._repositorio.crear(
    ...
    def _aplicar_filtro_debounced(self) -> None:
        ...
        if not self._filtro_busqueda:
            ...
            self._items = [
                usuario
                for usuario in self._items_completos
                if filtro in usuario.nombre_completo.lower()
                or filtro in usuario.email.lower()
                or (
                    usuario.corredor_numero
                    and str(usuario.corredor_numero).startswith(filtro)
                )
            ]
