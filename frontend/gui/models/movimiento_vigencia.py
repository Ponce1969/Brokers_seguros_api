...
class MovimientoVigencia:
    ...
    def from_dict(cls, data: dict) -> 'MovimientoVigencia':
        ...
        suma_asegurada = Decimal(str(data['suma_asegurada'])) if data.get('suma_asegurada') is not None else Decimal('0')
        prima = Decimal(str(data['prima'])) if data.get('prima') is not None else Decimal('0')
        comision = Decimal(str(data['comision'])) if data.get('comision') is not None else None

        return cls(
            id=data.get('id', 0),
            cliente_id=UUID(data['cliente_id']) if isinstance(data.get('cliente_id'), str) else data.get('cliente_id'),
    ...
    def vigente(self) -> bool:
        ...
        hoy = date.today()
        return (
            self.fecha_inicio <= hoy <= self.fecha_vencimiento and 
            self.estado_poliza.lower() == 'activa'
        )
